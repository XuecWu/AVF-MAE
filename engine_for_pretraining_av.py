...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
    ...
    for step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        decode_masked_pos_audio = decode_masked_pos_audio.to(device, non_blocking=True).flatten(1).to(torch.bool)
        ...
        with torch.cuda.amp.autocast():
            ...
            if use_frame_diff_as_target and target_diff_weight is not None:
                ...
                loss_video    = custom_mae_loss(outputs=outputs[0], labels=labels, bool_masked_pos=bool_masked_pos, decode_masked_pos=decode_masked_pos, B=B)


            loss_audio   = custom_mae_loss(outputs=outputs[1], labels=labels_audio, bool_masked_pos=bool_masked_pos_audio, decode_masked_pos=decode_masked_pos_audio, B=B_audio)
