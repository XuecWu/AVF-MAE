server=6000
pretrain_dataset='mix-data-109w-722-ft'
...
do
  for ckpt in "${ckpts[@]}";
  do
    OUTPUT_DIR="./saved/model/finetuning-mix-data-109w-722/${finetune_dataset}/audio_visual/${pretrain_dataset}_${model_dir}/checkpoint-${ckpt}/eval_split0${split}_lr_${lr}_epoch_${epochs}_size${input_size}_a${input_size_audio}_sr${sr}_server${server}"
    if [ ! -d "$OUTPUT_DIR" ]; then
      mkdir -p $OUTPUT_DIR
    fi
    DATA_PATH="./saved/data/${finetune_dataset}/audio_visual/split0${split}"
    MODEL_PATH="./saved/model/pretraining/mix-data-109w-four-parts_LGBlock-Dual-Masking-no-compile-719/audio_visual/${model_dir}/checkpoint-${ckpt}.pth"
