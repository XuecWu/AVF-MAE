...
do
  for ckpt in "${ckpts[@]}";
  do
    OUTPUT_DIR="./saved/model/finetuning-mix-data-109w-both-local-retest-806/${finetune_dataset}/audio_visual/${pretrain_dataset}_${model_dir}/checkpoint-${ckpt}/eval_split0${split}_lr_${lr}_epoch_${epochs}_size${input_size}_a${input_size_audio}_sr${sr}_server${server}"
