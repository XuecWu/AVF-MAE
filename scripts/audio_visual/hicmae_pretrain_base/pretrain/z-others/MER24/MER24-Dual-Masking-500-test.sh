...
OUTPUT_DIR="./saved/model/pretraining-test-dual-masking/MER24/audio_visual/${model}"
if [ ! -d "$OUTPUT_DIR" ]; then
  mkdir -p $OUTPUT_DIR
fi

DATA_PATH='./saved/data/MER24_pretrained_cropped/500_samples.csv'

OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES=7 python -m torch.distributed.launch --nproc_per_node=1 \
        --master_port 17327 \
