...
OUTPUT_DIR="./saved/model/pretraining/mix_data-701-20w-vox2/audio_visual/${model}"
if [ ! -d "$OUTPUT_DIR" ]; then
  mkdir -p $OUTPUT_DIR
fi
DATA_PATH='./saved/data/CN-Celeb-MER24-cropped_voxceleb2_CNCVS/CN-Celeb-MER24-cropped_voxceleb2_CNCVS_av_pretrain-20w-vox2.csv'
OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6 python -m torch.distributed.launch --nproc_per_node=7 \
        --master_port 11120 \
