model="AVF-MAE++_pretrain_base"
OUTPUT_DIR="./saved/model/pretraining-base-mix-data-119w-final-model-new-parms-929/audio_visual/${model}"
if [ ! -d "$OUTPUT_DIR" ]; then
  mkdir -p $OUTPUT_DIR
fi

DATA_PATH='./saved/data/mix-data-final-240927/cross-lingual-mix-data-overall-four-parts-selected-119w-240927.csv'

OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES=3,4,5,6 python -m torch.distributed.launch --nproc_per_node=4 \
        --master_port 17328 \
